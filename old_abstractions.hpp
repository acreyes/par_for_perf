...
#ifndef KOKKOS_ABSTRACTION_OLD_HPP_
#define KOKKOS_ABSTRACTION_OLD_HPP_
...
template <typename T> using buf_pool_t = ObjectPool<BufArray1D<T>>;
...
// Host mirrors
template <typename T> using HostArray0D = typename ParArray0D<T>::HostMirror;
template <typename T> using HostArray1D = typename ParArray1D<T>::HostMirror;
template <typename T> using HostArray2D = typename ParArray2D<T>::HostMirror;
template <typename T> using HostArray3D = typename ParArray3D<T>::HostMirror;
template <typename T> using HostArray4D = typename ParArray4D<T>::HostMirror;
template <typename T> using HostArray5D = typename ParArray5D<T>::HostMirror;
template <typename T> using HostArray6D = typename ParArray6D<T>::HostMirror;
template <typename T> using HostArray7D = typename ParArray7D<T>::HostMirror;
...
using device_view_t = Kokkos::View<multi_pointer_t<T, MAX_VARIABLE_DIMENSION>,
                                   Layout, DevMemSpace>;
template <typename T, typename Layout = LayoutWrapper>
...
par_dispatch(Pattern, const std::string &name, DevExecSpace exec_space,
...
par_dispatch(Pattern p, const std::string &name, DevExecSpace exec_space,
  ...
  par_dispatch<Tag>(p, name, exec_space, r.s, r.e, function,
...
template <typename, typename> class FlatFunctor;

template <typename F, typename... Args>
auto MakeFlatFunctor(F &function, Args... args) {
  return FlatFunctor<F, decltype(&F::operator())>(function,
                                                  std::forward<Args>(args)...);
}

template <typename Function, typename R, typename T, typename Index,
          typename... FArgs>
...
public:
  FlatFunctor(const Function _function, const int _NjNi, const int _Ni,
...
template <typename Function, typename R, typename T, typename Index,
          typename... FArgs>
class FlatFunctor<Function,
                  R (T::*)(Index, Index, Index, Index, FArgs...) const> {
  int NkNjNi, NjNi, Ni, nl, kl, jl, il;
  Function function;

public:
  FlatFunctor(const Function _function, const int _NkNjNi, const int _NjNi,
              const int _Ni, const int _nl, const int _kl, const int _jl,
              const int _il)
      : function(_function), NkNjNi(_NkNjNi), NjNi(_NjNi), Ni(_Ni), nl(_nl),
...
inline typename std::enable_if<sizeof...(Args) <= 1, void>::type
par_dispatch(LoopPatternMDRange, const std::string &name,
             DevExecSpace exec_space, const int ll, const int lu, const int ml,
             const int mu, const int nl, const int nu, const int kl,
             const int ku, const int jl, const int ju, const int il,
             const int iu, const Function &function, Args &&...args) {
  Tag tag;
  kokkos_dispatch(tag, name,
...
inline void par_dispatch(LoopPatternFlatRange, const std::string &name,
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int &idx) {
        ...
        int j =
            (idx - l * NmNnNkNjNi - m * NnNkNjNi - n * NkNjNi - k * NjNi) / Ni;
        int i = idx - l * NmNnNkNjNi - m * NnNkNjNi - n * NkNjNi - k * NjNi -
                j * Ni;
        l += ll;
        m += ml;
        n += nl;
...
template <class... Args> inline void par_scan(Args &&...args) {
...
par_for_inner(InnerLoopPatternTTR, team_mbr_t team_member, const int ll,
              const int lu, const int ml, const int mu, const int nl,
              const int nu, const int kl, const int ku, const int jl,
...
template <typename Function>
KOKKOS_FORCEINLINE_FUNCTION void
par_for_inner(InnerLoopPatternTTR, team_mbr_t team_member, const int ml,
              const int mu, const int nl, const int nu, const int kl,
              const int ku, const int jl, const int ju, const int il,
              const int iu, const Function &function) {
  const int Nm = mu - ml + 1;
  ...
  const int NmNnNkNjNi = Nm * NnNkNjNi;
  Kokkos::parallel_for(
      Kokkos::TeamThreadRange(team_member, NmNnNkNjNi), [&](const int &idx) {
        int m = idx / NnNkNjNi;
...
template <typename Function>
KOKKOS_FORCEINLINE_FUNCTION void
par_for_inner(InnerLoopPatternTTR, team_mbr_t team_member, const int il,
              const int iu, const Function &function) {
  Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, il, iu + 1),
                       function);
}
...
// Thus, we need to pass `SquentialHostInit` as allocator, but only if the
// ViewOfView is on the host. If the ViewOfViews in on the device, then
// `SequentialHostInit` should be passed when calling `create_mirror_view`,
// which is automatically handled by the create_view_of_view_mirror() function
// below.
...
template <typename T> auto create_view_of_view_mirror(T view) {
  return Kokkos::create_mirror_view(
      Kokkos::view_alloc(Kokkos::SequentialHostInit), view);
}
